...
export default function ReservesProductionSection() {
  ...
  const handleRomChange = useCallback(
    ...
    [updateInput, isFree, romLimitReached, hasIncrementedThisSession, incrementRomUsage]
  );
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(e) => updateInput('oreReserves', parseFloat(e.target.value) || 0)}
            />
          </div>
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(e) => updateInput('oreGrade', parseFloat(e.target.value) || 0)}
            />
          </div>
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(e) => updateInput('recoveryRate', parseFloat(e.target.value) || 0)}
            />
          </div>
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(e) => updateInput('strippingRatio', parseFloat(e.target.value) || 0)}
            />
          </div>
        </div>
      </div>
        ...
        <Label className="flex items-center gap-2">
          ...
          {isFree && (
            <span className="ml-auto text-xs text-muted-foreground font-normal">
              {Math.min(romUsageCount, FREE_TIER_ROM_LIMIT)}/{FREE_TIER_ROM_LIMIT} uses
            </span>
          )}
        </Label>
          ...
          <div className="space-y-3">
            ...
            <Alert className="border-amber-500/50 bg-amber-50/50 dark:bg-amber-950/20">
              ...
              <AlertDescription className="text-amber-700 dark:text-amber-300 space-y-3">
                <p>
                  You have used all <strong>{FREE_TIER_ROM_LIMIT} free ROM Tonnage Schedule edits</strong>.
                  Upgrade to Premium to continue editing the Annual ROM Tonnage Schedule without restrictions.
                </p>
                <Button
                  size="sm"
          ...
          <div className="space-y-1">
            {isFree && romUsageCount > 0 && (
              <p className="text-xs text-muted-foreground">
                {FREE_TIER_ROM_LIMIT - romUsageCount} free {FREE_TIER_ROM_LIMIT - romUsageCount === 1 ? 'edit' : 'edits'} remaining
              </p>
            )}
            <DynamicArrayInput
              values={inputs.romTonnageSchedule}
