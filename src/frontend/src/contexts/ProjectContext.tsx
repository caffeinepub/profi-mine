...
import {
  ...
  useIncrementRomUsage,
  useLoadProject as useLoadProjectMutation,
...
interface ProjectContextType {
  ...
  updateInput: <K extends keyof ProjectInputs>(
    key: K,
...
function getUsageLimit(tier: SubscriptionTier | undefined): number {
  ...
  if (tier.__kind__ === "premium")
    return Number(tier.premium.MAX_OPERATIONS_PDF_AND_CSV);
  if (tier.__kind__ === "basic")
    return Number(tier.basic.MAX_OPERATIONS_PDF_AND_CSV);
...
export function ProjectProvider({ children }: { children: ReactNode }) {
  ...
  const updateInput = <K extends keyof ProjectInputs>(
    key: K,
    value: ProjectInputs[K],
  ) => {
  ...
  const loadProject = useCallback(
    async (id: Uint8Array) => {
      ...
      const lom =
        project.lom != null && project.lom > 0
          ? Math.round(project.lom)
          : DEFAULT_INPUTS.romTonnageSchedule.length;
      const numYears = Math.max(1, Math.min(lom, 50));

      // Reconstruct romTonnageSchedule: fill with the saved average romTonnage
      const romTonnage =
        project.romTonnage > 0
          ? project.romTonnage
          : DEFAULT_INPUTS.romTonnageSchedule[0];
      ...
      const commodityPrice =
        project.commodityPrice > 0
          ? project.commodityPrice
          : DEFAULT_INPUTS.commodityPrices[0];
      ...
      const restoredInputs: ProjectInputs = {
        oreReserves:
          project.oreReserves > 0
            ? project.oreReserves
            : DEFAULT_INPUTS.oreReserves,
        oreGrade:
          project.oreGrade > 0 ? project.oreGrade : DEFAULT_INPUTS.oreGrade,
        // recoveryRate is stored as a fraction (0-1) in the backend, convert back to percentage
        recoveryRate:
          project.recoveryRate > 0
            ? project.recoveryRate <= 1
              ? project.recoveryRate * 100
              : project.recoveryRate
            : DEFAULT_INPUTS.recoveryRate,
        strippingRatio:
          project.strippingRatio >= 0
            ? project.strippingRatio
            : DEFAULT_INPUTS.strippingRatio,
        romTonnageSchedule,
        commodityPrices,
        ...
        discountRate:
          project.discountRate > 0
            ? project.discountRate <= 1
        ...
        taxRate:
          project.averageTaxRate > 0
            ? project.averageTaxRate <= 1
              ? project.averageTaxRate * 100
              : project.averageTaxRate
            : DEFAULT_INPUTS.taxRate,
        initialCapex:
          project.capex > 0 ? project.capex : DEFAULT_INPUTS.initialCapex,
        // sustainingCapex is not stored separately; keep default
        sustainingCapex: DEFAULT_INPUTS.sustainingCapex,
        miningCost:
          project.miningCost > 0
            ? project.miningCost
            : DEFAULT_INPUTS.miningCost,
        processingCost:
          project.processingCost > 0
            ? project.processingCost
            : DEFAULT_INPUTS.processingCost,
        gAndACost:
          project.gAndACost > 0 ? project.gAndACost : DEFAULT_INPUTS.gAndACost,
